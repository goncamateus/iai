...
def begin_graphics(width=640, height=480, color=formatColor(0, 0, 0), title=None):
    ...
    try:
        _canvas = Canvas(_root_window, width=width, height=height)
        _canvas.pack()
        draw_background()
        _canvas.update()
    except:
...
def image(pos, file="../../blueghost.gif"):
    ...
    return _canvas.create_image(x, y, image = PhotoImage(file=file), anchor = NW)
...
if __name__ == '__main__':
    ...
    move_to(g, (50, 50))
    circle((150, 150), 20, formatColor(1, 1, 1), formatColor(0.7, 0.3, 0.0), endpoints=[15, - 15])
    sleep(2)
